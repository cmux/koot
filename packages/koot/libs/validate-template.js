 ...
 */
const validateTemplate = async (optionTemplate) => {
    ...
    if (fs.existsSync(optionTemplate)) {
        template = await fs.readFile(optionTemplate, 'utf-8')
    } else if (optionTemplate.substr(0, 2) === './') {
        template = await fs.readFile(path.resolve(getCwd(), optionTemplate), 'utf-8')
    } else if (path.isAbsolute(optionTemplate)) {
        template = await fs.readFile(path.resolve(optionTemplate), 'utf-8')
    }
            ...
            Object.keys(important[tagName]).forEach(type => {
                important[tagName][type].forEach(inject => {
                    ...
                    template = template.replace(
                        new RegExp(`<${type == 'append' ? '/' : ''}${tagName}.*?>`),
                        (match) => {
                            if (type === 'prepend')
                                return `${match}\n${str}`
                            return `${str}\n${match}`
