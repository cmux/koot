...
function AddReactHotLoader(source) {
    ...
    {
        const newSource = require('./transforms/export-default-named-class-component')(
            theSource
        );
        if (newSource !== theSource) return newSource;
    }
    ...
    if (className) {
        ...
        theSource = transformSourceForClassExported(
            theSource,
            classNameExported
        );
        ...
        theSource = require('./transforms/export-anonymous-functional-component-with-hoc')(
            theSource
        );
...
function transformSourceForClass(source, className) {
    ...
    const regex = new RegExp(
        `^\\s*export\\s+default\\s+class\\s+${className}\\s+`,
        'm'
    );
...
function getImportLine() {
    ...
    return `import hot from '${customHotLoader}';\n`;
}
...
function getExportDefaultFunctionName(source) {
    ...
    const matches = source.match(
        /^\s*export\s+default\s+function\s+([^(\s^;]*)\s?\(/m
    );
