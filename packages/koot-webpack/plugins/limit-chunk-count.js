...
const schema = require('webpack/schemas/plugins/optimize/LimitChunkCountPlugin.json');
...
/** @typedef {import("webpack/declarations/plugins/optimize/LimitChunkCountPlugin").LimitChunkCountPluginOptions} LimitChunkCountPluginOptions */
            ...
            (compilation) => {
                compilation.hooks.optimizeChunks.tap(
                    ...
                    (chunks) => {
                        ...
                        let remainingChunksToMerge =
                            compilation.chunks.size - maxChunks;

                        // order chunks in a deterministic way
                        const compareChunksWithGraph = compareChunks(
                            chunkGraph
                        );
                        const orderedChunks = Array.from(chunks).sort(
                            compareChunksWithGraph
                        );
                        ...
                        orderedChunks.forEach((b, bIdx) => {
                            ...
                            for (let aIdx = 0; aIdx < bIdx; aIdx++) {
                                ...
                                const aSize = chunkGraph.getChunkSize(
                                    a,
                                    options
                                );
                                const bSize = chunkGraph.getChunkSize(
                                    b,
                                    options
                                );
                                const c = {
                        ...
                        loop: while (true) {
                            ...
                            // merge the chunks
                            const canIntegrate = chunkGraph.canChunksBeIntegrated(
                                a,
                                b
                            );
                            if (canIntegrate) {
                                ...
                                for (const combination of combinationsByChunk.get(
                                    a
                                )) {
                                ...
                                for (const combination of combinationsByChunk.get(
                                    ...
                                    if (combination.a === b) {
                                        if (
                                            !chunkGraph.canChunksBeIntegrated(
                                                a,
                                                combination.b
                                            )
                                        ) {
                                        ...
                                        const finishUpdate = combinations.startUpdate(
                                            combination
                                        );
                                        combination.a = a;
