 ...
 */
module.exports = async (dir = process.cwd(), options = {}) => {
    const {
        onlyASCII = false,
        ignore = [],
    ...
    return (await glob('**/*', {
        ...
        .filter(pathname => {
            ...
            if (onlyASCII && isBinaryFile.sync(pathname))
                return false
...
const getIgnores = async (dir = process.cwd()) => {
    ...
    if (!fs.existsSync(pathnameIgnore))
        return []
    ...
    return parseIgnore(pathnameIgnore)
        .map(pattern => {
            if (pattern.substr(0, 1) === '/') {
                if (pattern.substr(pattern.length - 2, 2) === '**' &&
