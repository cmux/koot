...
const addKootVersionToPackageJson = require('../../../../packages/koot-cli/lib/modify-package-json/add-koot-version');
...
describe('测试: 修改 package.json', () => {
    ...
    test(`若在 dir 中未找到 package.json，应报错`, async () => {
        let error;
        await modifyDependency(path.resolve(__dirname, '../../'), 'koot').catch(
            err => (error = err)
        );
    ...
    test(`若没有提供 moduleName，应报错`, async () => {
        let error;
        await modifyDependency(__dirname).catch(err => (error = err));
    ...
    test(`添加新的依赖，未提供版本号：依赖不存在，应报错`, async () => {
        ...
        let error;
        await modifyDependency(__dirname, m).catch(err => (error = err));
